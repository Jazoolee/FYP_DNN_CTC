...
Usage:
    python3 plot_comparison.py <dataset_file> <continuous_log_file> [pos_threshold] [tau_threshold]
...
Generates a single high-resolution PNG file with 2x3 grid layout:
...
def main():
    if len(sys.argv) < 3 or len(sys.argv) > 5:
        print("Usage: python3 plot_comparison.py <dataset_file> <continuous_log_file> [pos_threshold] [tau_threshold]")
        print("\nExample:")
    ...
    if len(sys.argv) >= 4:
        POS_THRESHOLD = float(sys.argv[3])
    if len(sys.argv) >= 5:
        TAU_THRESHOLD = float(sys.argv[4])
